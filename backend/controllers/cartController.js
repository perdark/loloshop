...
async function addItem(req, res) {
  const { product_id, selections, qty, design_id } = req.body;
  ...
  const itemId = await tx(async (client) => {
    ...
    if (designId) {
      ...
      if (existing.rows.length) {
        await client.query(
          `UPDATE cart_items SET product_id = $1, selections = $2, price_snapshot = $3, qty = $4 WHERE id = $5`,
          [product_id, selJson, priced.total, quantity, existing.rows[0].id]
        );
    ...
    const { rows } = await client.query(
      `INSERT INTO cart_items (cart_id, product_id, design_id, selections, price_snapshot, qty)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`,
      [cartId, product_id, designId, selJson, priced.total, quantity]
    );
    await client.query(`UPDATE carts SET updated_at = NOW() WHERE id = $1`, [cartId]);
    return rows[0].id;
  });
...
// an order: customizable products (sash) go to 'designing' so the student designs them next;
// everything else enters the preparer's queue ('preparing'). Cart is cleared on success.
async function checkout(req, res) {
  ...
  const itemsRes = await query(
    `SELECT ci.id, ci.product_id, ci.design_id, ci.selections, ci.qty, p.customizable
     FROM cart_items ci JOIN products p ON p.id = ci.product_id
     WHERE ci.cart_id = $1 ORDER BY ci.created_at`,
    [cartId]
  );
  ...
  // Single-item checkouts leave bundleId null — no bundle linkage needed.
  ...
  const orderIds = await tx(async (client) => {
    ...
    for (const { ci, priced } of lines) {
      ...
      if (ci.design_id) {
        ...
        // Reconcile with the draft order auto-created while designing (keyed by design_id)
        // so checkout never produces a duplicate sash order.
        await client.query(
        ...
        if (existing.rows.length) {
          ...
          await client.query(
            `UPDATE orders SET price = $1, status = 'design_complete', notes = $2, checkout_group_id = $3 WHERE id = $4`,
            [priced.total, note, bundleId, oid]
          );
          ...
          const o = await client.query(
            `INSERT INTO orders (student_id, product_id, design_id, price, status, notes, checkout_group_id)
             VALUES ($1, $2, $3, $4, 'design_complete', $5, $6) RETURNING id`,
            [student.id, ci.product_id, ci.design_id, priced.total, note, bundleId]
          );
          oid = o.rows[0].id;
        }
      } else {
        // Non-design products (robe/cap/shawl) enter the preparer's queue directly.
        // Respect uq_orders_student_product_nodesign by reusing any existing no-design order.
        const status = ci.customizable ? 'designing' : 'preparing';
        const existing = await client.query(
        ...
        if (existing.rows.length) {
          oid = existing.rows[0].id;
          await client.query(
            `UPDATE orders SET price = $1, status = $2, notes = $3, checkout_group_id = $4 WHERE id = $5`,
            [priced.total, status, note, bundleId, oid]
          );
          await client.query(`DELETE FROM order_items WHERE order_id = $1`, [oid]);
        } else {
          const o = await client.query(
