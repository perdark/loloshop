...
### ٤) «الخط العربي» انفتح لمحمد عماد

طلب المالك بنفس الجلسة. البوابة `allowCalligraphyUser` (`routes/calligraphy.js`) تفتح لـ
**admin · manager · designer · عضو فريق تصميم فعّال** — ومحمد عماد `embroiderer`. الحل **بيانات
مو كود**: عمود `staff_types` مصفوفة ويدعم دورين أصلاً (امنة عندها `designer + manager`)، فصار
`['embroiderer','designer']`. متحقَّق حيّاً بتوكنه: `/staff/calligraphy` تفتح، والشريط الجانبي
صار يكَرا «تطريز · مصمم» وظهر فيه «الخط العربي».
⚠️ **هذا انطبق على قاعدة التطوير بس.** على الإنتاج هو **ضغطة وحدة بـ`/staff/team`** (اختيار
نوعين للموظف) — ما يحتاج جلسة ولا deploy.
⚠️ **`designer` يفتح أكثر من أداة الخط**، والمالك لازم يعرف: `QUEUE_STAGES.designer =
['design_complete']` يعني «مرحلتي» عنده تصير **التصميم + التطريز**، و`requireDesignerOrAdmin`
يعني يكدر يضغط «تحويل للتطريز» (`sendOrder`). إذا المقصود أداة الخط بس، هذا يحتاج نوع موظف
جديد أو شرط ثالث بالبوابة — مو تعديل بيانات.
