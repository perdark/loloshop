...
## 2026-09-26 — 💰 «الداشبورد ما تنطي الأرباح صح» → التكاليف والربح الحقيقي (`feat/true-costs`، ما اندمج)

**السبب (مقاس على نسخة prod المحلية، لحد 14 أيلول):** كل «ربح» بالنظام كان من `orders.price`/`orders.cost` بس. و`orders.cost` مو تكلفة إنتاج، لأنه بطلبات الممثل حصة الإدارة وبالتجزئة NULL. الرواتب وأجور الورشة وصرف الذكاء الاصطناعي ما تنطرح من أي مكان، وماكو جدول مصاريف أصلاً. 6 موظفين بلا راتب مسجل، وأجور خياطة تموز غير مسجلة (0 من 1182).

**التعديل:**
- **Migration 113** (`db/migrations/113_cost_model.sql`، مكررة بـ `schema.sql`): `cost_items` (قائمة الأسعار)، و`product_cost_lines` (وصفة المواد لكل نوع + إضافات لمنتج محدد)، و`shop_expenses` (شهري / لمرة واحدة / خسارة). الـseed تقديري (`confirmed=false`) وينزرع **مرة وحدة بس** عن طريق marker بـ `site_settings` (`cost_model_seeded`)، حتى اللي تحذفه الإدارة ما يرجع بالـdeploy الجاي.
- `lib/trueProfit.js` `computePnl`: دخل المحل (من counts.js نفسه) − المواد − أجور الورشة − الرواتب (الكشف ← الراتب الأساسي ← أيام الدوام × أجر اليوم التقديري) − الشهري − لمرة واحدة − الخسائر (يدوي + مواد الطلبات الملغاة بعد الشغل) − الذكاء الاصطناعي. الشهر الحالي ينحسب بنسبة الأيام اللي مضت. طقم الممثل والباقة ينحسبون كطقم، لأن الفلوس على صف الوشاح.
- `/api/admin/costs/*`: قراءة، وإضافة/تعديل/حذف لكل البنود والوصفات والمصاريف، والإعدادات (سعر الصرف 1450، أجر اليوم 16,600).
- الواجهة `/admin/costs` بأربع تبويبات، وبطاقة «صافي الربح التقريبي — هذا الشهر» بالداشبورد.
- «لولو الإدارة»: metric جديد `true_profit`، وبطاقتين اقتراح (الشهر خسران / بنود تقديرية). ونص `revenue_summary` صار يقول «إيراد مو ربح» دائماً. و`salary_summary` صار يسمي `salary_set` صح.
- **منع مسح حصة الإدارة:** `PATCH /admin/orders/:id/cost` يرجع 409 على طلب ممثل، وكذلك `set_order_cost` بالذكاء الاصطناعي (اللي كان مكسور أصلاً: `p.name` و join غلط).
- `/admin/orders`: خانة «إجمالي التكلفة» بعرض الباقات كانت تعرض الدخل، واتصححت. وحقل «تعديل التكلفة» انشال من صفوف الممثلين.

**النتيجة (حزيران لحد 14 أيلول، كله تقديري):** دخل 153.8M − مواد 34.1M − رواتب 7.7M − ثابت 8.7M − ورشة 4.2M − ذكاء اصطناعي 0.08M − خسائر 0.05M = **صافي ≈ 99M (64%)**.

**تحقق:** `test/costModel.test.js` 8/8، منها تست إن الدخل يساوي `settledMoney` بالضبط، وتست إن الـseed ما يرجّع شي انحذف. `schema.sql` كامل انطبق مرتين بدون تغيير. tsc و eslint نظيفين. الصفحة انفتحت بالمتصفح: تعديل سعر + «مؤكد» انحفظ بالقاعدة، وعرض 375px بدون سكرول أفقي. الفشلات بالـsuite الكامل (assemblyBoard، pushBroadcast، app-open) تفشل بـ main النظيف هم.

**دفتر الجلسة ويا الإدارة:** artifact «دفتر تكاليف لولو شوب». **باقي:** الإدارة تأكد الأسعار. ⚠️ **لازم migration 113 تنطبق بنفس الـdeploy** (موجودة بـ `schema.sql`، فـ deploy.sh يغطيها)، وإلا `/admin/costs` و`true_profit` يطلعون 500.

## 2026-09-24 — ✍️ «ألواح الممثلين مرة ثلث ومرة نسخ، والمفرد ما بيه ثلث أبداً»

**مقاس على ألواح prod الحقيقية (اليوم + 13–20 أيلول):** البرومبت يذكر «Thuluth» مرة وحدة بس، فالموديل يرجع للنسخ متى ما يريد: ثلث بسطر، ونسخ عريض بالي بعده، ونسخ رفيع بالثالث. الألواح المفردة كلها نسخ. السبب إن قاعدة 09-17 «NO stroke dropping below its line» تمنع الذيول والتركيب، واللي هي الثلث نفسه. وفوكاها: الموديل رسم الـ`- ` اللي بقائمة الأسماء كشرطة جنب الاسم («- الاقتصادي حيدر هاشم»، «-الدكتور حسين عباس-»)، وحط توقيع خطاط وهمي على لوح.

**⚠️ تصحيح بنفس اليوم — النشر الأول لفّ الأسماء:** جملة «letters stacked and composed over each other» خلّت الموديل يطوي الاسم لمربع. مقاس على prod: عرض÷ارتفاع اللوح نزل من وسيط **5.6** (330 مفرد قبل النشر) إلى **2.2**، ولوحين من أول 4 حطّوا اللقب (الفاحصه/التقني) بسطر لحاله، ولوح وحد انعاد 4 مرات (0.32$). اتشالت جملة التركيب، وانضاف «سطر واحد، لا لقب فوك الاسم، والعرض 4 أضعاف الارتفاع على الأقل». التجربة (0.64$): 10/10 بسطر واحد، والقص 10/10، والنسبة 2.7–4.9. نسبة 21:9 للمفرد ما حسّنت على 1:1، فبقت 1:1. **الرقم اللي لازم ينقاس بعد أي تعديل هو نسبة العرض للارتفاع، مو العين بس.**

**التعديل (`fix/calligraphy-thuluth-lock`، بموافقة المدير):**
- `BASE`: «genuine classical THULUTH — NOT Naskh, NOT Ruq'ah, NOT Diwani, NOT a printed or typed font» + وصف الثلث (ألفات بالرأس المعقوف، تركيب، ذيول ممدودة). بدون أي كلمة عن الحركات (الـlandmine باقي).
- قاعدة التطريز خفّت: الاسم تركيبة وحدة على **سطر واحد** وتبقى جوه حزمتها، وما يضيع ولا حرف بالتركيب.
- `NEG`: لا شرطة/نقطة قبل الاسم أو بعده، ولا توقيع/ختم/تاريخ. وقائمة الأسماء صارت بدون `- `.
- `calligraphyStyles.plain`: شلنا «plain letters only»، والزخارف بس اللي تنشال.
- التستات: 10/10، وتستين جداد (القاعدة المسطّحة ممنوعة، والنسخ والتوقيع والـbullet ممنوعين).
- **جُرّب بصور حقيقية على السيرفر (~0.51$):** ورقة 5 ممثلين كلها نفس الخط، بلا شرطة، والقص 5/5. الجولة الأولى من المفرد لفّت «المحامية» لسطر ثاني وضيّعت ع «اسماعيل»، فانضاف «سطر واحد + كل حرف موجود». بالجولة الثانية طلعت الثلاث ثلث بسطر واحد وإملاء كامل، والقص 1/1.

## 2026-09-23 — ✍️ «ألواح الممثلين تطلع خط عادي بدون زخرفة، والتجزئة زينة»

**السبب (مقاس على نسخة prod المحلية):** 94% من ألواح الممثلين تطلع من أوراق بيها 7–10 أسماء، يعني كل اسم ~200px بورقة 2K 9:16، وبهالحجم الموديل يرسم فونت مو ثلث. التجزئة/المكتوبة أغلبها 1–6 أسماء بالورقة. وفوقها سطر `MINIMAL` بالبرومبت (أمام وخلف الوشاح) كان يگول «clean plain letters»، فيوصل للحروف نفسها.

**التعديل (على main):**
- `lib/calligraphyBatching.js` `FULL_SHEET` 10 → **5**، و`calligraphyEngine.js` صار `BATCH = FULL_SHEET` (رقم واحد، حتى الانتظار والدفعة يتفقون). يعني ~410px للاسم، ~0.02$ بدل ~0.01$.
- `lib/calligraphyPrompt.js` `MINIMAL`: شلنا «plain letters» — القيد على الزخارف بس، والحروف تبقى خط كامل.
- الواجهة: `MIN_BATCH` 10 → 5 بـ `frontend/lib/calligraphy.ts` + نص «عشرة» ← «خمسة».
- تستات: `calligraphyBatching` صارت تعتمد على `FULL_SHEET`، و`calligraphyCost` (تست التعبئة) صار 2+2 بدل 3+4. كل تستات calligraphy 67/67، tsc نظيف.
- **جُرّب بصورة حقيقية (0.20$):** نفس 5 أسماء ممثلين — قبل خط عادي ~200–240px، بعد ثلث مزخرف ~300–400px، القص 10/10 و 5/5.

**باقي:** نص الطالب يوصل خام («اسم تبارك…» انرسمت كلمة «اسم») — مشكلة منفصلة.

## 2026-09-15 (ب) — ✍️ «الخط رجع عادي مو مزخرف» — البرومبت انكسر بالاتجاه الثاني

**الطلب:** «ليش الخط صار يولد صور بخطوط عادية وليس مزخرفة؟ مو مثل زمان».

**الجواب: برومبت أمس.** `7a7e5ee` (٢٠٢٦-٠٩-١٤) صلّح مشكلة حقيقية — «masterful diacritics»
كانت ترجّع الأسماء مشكّلة وتجرّ وراها حروف زايدة (٠ من ٥ نظيفة) — بس الإصلاح **شال الكلمة
الوحيدة بكل البرومبت اللي تطلب فن**، وحطّ مكانها منع:

| | قبل ٠٩-١٤ | بعد ٠٩-١٤ |
|---|---|---|
| وصف الفن | «masterful» | ✗ انشالت |
| وزن القلم | thick/thin contrast | «THICK, BOLD … **never thin**, faint, wiry or hairline» |
| التشكيل | يُطلَب | ٩ نفيات متتالية |
| الزخرفة (`MINIMAL`) | «mostly **clean plain letters**» | نفسها |

اللي بقى يوصف: حروف عربية **ثقيلة، عارية، بلا زخرفة، على أبيض**. هذا وصف **خط طباعة عريض**،
مو وصف ثلث. والموديل رسم اللي انطلب منه.

**فخّان محددان، وهذا هو الجذر:**
1. **تناقض صريح.** «strong thick/thin contrast» + «never thin … or hairline» ما يجتمعون:
   **الرفيع هو نص التباين** — القلم العريض يسوّيه لحاله كل ما يلتف. منع الرفيع يسطّح كل
   الجرّات على وزن واحد، ووزن واحد = شكل الفونت بالضبط.
2. **ماكو ولا كلمة تكَول له شنو ما يصير يشبه.** «NOT a computer font, NOT a typeface» ما
   كانت موجودة أبداً — مع إن هذا بالذات هو الخلل المُبلَّغ عنه.

**الإصلاح (`backend/lib/calligraphyPrompt.js`) — الاتجاهان سوا، بلا ما ندوس ولا وحدة:**
- الفن **أول وأطول**: «masterpiece of classical Thuluth … master calligrapher … sweeping
  curves, dramatic thick-to-thin modulation, tapered endings, generous kashida, letters that
  lean, stack and interlock» + النفي الصريح «NOT a computer font, NOT a typeface».
- **قاعدة الوزن انحددت بالجرّات العريضة بس**: «the broad strokes are pressed HEAVY … never a
  faint, weak or wiry line — while the fine strokes the nib makes as it turns stay sharp».
  يعني ضغط القلم مال ٠٩-١٤ **باقي** (الورقة الرفيعة ماكو بيها شي يملاه الـdigitizer)، بس
  التباين رجع.
- **منع التشكيل باقي كلمة بكلمة ومعدود** — هو المقيس، والباراڤريز ما انفحص.
- **`MINIMAL` انحدد نطاقها بالزخارف بس.** قرار المدير مال ٢٠٢٦-٠٨-٢٦ (الوجه مثل الظهر)
  **ما تغيّر** — بس صيغته «keep it mostly clean plain letters» كانت تحچي عن **الحروف** مو
  عن الزخارف، وصارت ثالث جملة بنفس البرومبت تكَول «خلّي الحروف عادية».

**الحارس:** `backend/test/calligraphyPrompt.test.js` — ٨ تستات، بلا شبكة وبلا DB وبلا صورة
مدفوعة. فحص أحمر/أخضر: على برومبت أمس **٤ يطيحون**، وبعد الإصلاح **٨/٨**، والأربعة اللي
ظلّوا ناجحين بالحالتين هم تستات التشكيل وضغط القلم — دليل إن إصلاح ٠٩-١٤ ما انتقض.

**⚠️ وأكو نص ثاني للقصة، ما انلمس عمداً — قرار المدير:** حجز الأوراق (`bede219`، نفس
الليلة) خلّى الأسماء اللي كانت تنطبع لحالها تنتظر وتدخل بورقة عشرة. والورقة **2K 9:16**
تعطي كل اسم **~٢٠٠px ارتفاع**، بينما الاسم اللحاله يشتري **1K 1:1 = ١٠٢٤px**. يعني نفس
الاسم اللي كان يجي بخمس أضعاف التفاصيل صار يجي بخُمسها — وبـ٢٠٠px الموديل **ما يكدر**
يرسم تدرّج الرفيع/العريض حتى لو طلبناه. **ما غيّرناها**: الحجز يوفّر ~٣٥٪ من الفاتورة،
وتغييره قرار مدير مو قرار جلسة. المفاتيح موجودة كلها: «ولّدها هسة» (`force`) بالورشة تعطي
الاسم ورقته لحاله فوراً · `CALLIG_HOLD_MINUTES=0` يطفي الحجز للمحل كله · أو `BATCH` من ١٠
لـ٥ (~٤١٠px للاسم، الكلفة تصير ~$0.02 للاسم بدل ~$0.01).

**ما انفحص حي:** ماكو `OPENROUTER_API_KEY` ولا `node_modules` بهذي الجلسة، فالإصلاح مفحوص
**كنص برومبت**، مو كصورة راجعة من الموديل. **أول ورقة على الإنتاج لازم تنشاف بالعين** —
والشي اللي ينراد يتأكد منه تحديداً: رجع التزويق **بلا** ما يرجع التشكيل.

## 2026-09-15 — 💸 الورقة تنتظر حتى تمتلئ (batching hold)

**الطلب:** «خلّيها أرخص».

**القياس اللي بنى القرار** (نسخة الإنتاج، ٢٠٢٦-٠٨-٠١ → ٠٩-١٢):

| امتلاء الورقة | أوراق | أسماء | كلفة | للاسم |
|---|---|---|---|---|
| ٧–١٠ | ١٢٦ | ١٬٢١٩ | $28.11 | **$0.0231** |
| ٤–٦ | ٦٠ | ٢٨٢ | $7.48 | $0.0265 |
| ٢–٣ | ١٠٨ | ٢٤٤ | $12.42 | $0.0509 |
| **اسم واحد** | **٢٨٣** | **٢٨٣** | **$26.22** | **$0.0927** |

**نص الأوراق اللي انطبعت كانت تحمل اسم واحد** — ٣٥٪ من الفاتورة مقابل ١٤٪ من الأسماء،
بأربع أضعاف سعر الورقة الكاملة. وبأيلول صار أسوأ: ٢٫٦ اسم بالورقة مقابل ٦٫٩ بتموز.

**والسبب مو الـrerolls** (٢٥ من الـ٢٨٣ بس كانت إعادة توليد). السبب بنيوي: الـcross-job
top-up مال ٢٠٢٦-٠٨-١٨ ما يكدر يستعير إلا لوحات `pending` **بنفس اللحظة**، والمحل يشتغل
ممثل ممثل — فالبركة اللي يستعير منها فاضية دائماً. ما كان بالـtop-up خلل؛ ما كان أكو
طابور يسحب منه.

**`lib/calligraphyBatching.js` هو الطابور:** الورقة الناقصة **تنحجز** بدل ما تنشرى،
وتنطبع لمن تمتلئ (١٠) أو لمن يخلص وقت أقدم اسم عليها (`CALLIG_HOLD_MINUTES`، **١٠ دقائق**
افتراضياً بالكود — ما يحتاج env بالإنتاج). اللوحة تبقى `pending`، وهي الحالة الوحيدة اللي
كل المحرك أصلاً يفهمها بـ«أكو أحد راح يلكَطها» (سقف الميزانية وإصلاح انقطاع ٠٨-٢٨ كلهم
يعتمدون عليها).

⚠️ **الموعد ينقاس من أقدم اسم مو من آخر واحد وصل.** لو انقاس من آخر واحد، وصول أسماء
بالتقطير يدزّ الاسم الأول للورا للأبد — الطالب اللي وصل أول يصير آخر واحد ينطبع.
⚠️ **الحجز مو تعليق، وworker.js لازم يفرّق.** `handleGeneration` يرمي على «تقدّم صفر مع
شغل باقي»، والحجز يعطي نفس الشكل — فلو انرمى، تنحرق محاولتا pg-boss بـ٩٠ ثانية وبعدين
**تنهجر اللوحات**. عشان هيك `held` حقل صريح مو استنتاج.
⚠️ **ولازم يبقى ممكن تتخطاه:** «ولّدها هسة» بالورشة (`force`)، و`CALLIG_HOLD_MINUTES=0`
مفتاح إطفاء للمحل كله بدون نشرة. قاعدة ما تنتخطى = سبب يوقفون يستعملون الشاشة.
⚠️ **`lib/calligraphySweep.js` شبكة أمان كل دقيقة:** الحجز ينصحى برسالة مؤجّلة وحدة،
و`singletonKey` ما يحمي إلا وهي بـ`created` — فإعادة تشغيل الـworker تكدر تبلعها، واللوحة
تكَعد `pending` للأبد وأحد ما ينطرها. العرض بالشاشة يسأل **نفس** `holdDecision` مو قاعدة
ثانية (٦ فرونت + ٦ باك = ورقتين ناقصات مو ورقة كاملة).

**فحص حي:** اسم واحد → محجوز، $0 · وصلوا ١٠ أسماء عبر **وظيفتين مختلفتين** → ورقة وحدة،
$0.101 لـ١٠ أسماء = **$0.0101 للاسم** مقابل $0.0927 قبل (٩ أضعاف أرخص). والـsweep لكَه
الوظيفة المتأخرة. **المتوقع: ~٣٥٪ من فاتورة الخط، ~$١٨-٢٠ بالشهر بالحجم الحالي.**
⚠️ وانحذف تست كان يشتري صورة حقيقية كل تشغيلة ($0.067، ١١ ثانية) — بملف تستات موجود
أصلاً حتى يوقف شراء الأوراق الناقصة.

---

## 2026-09-14 (ب) — 🔕 وقّفنا إشعارات المراحل · ✏️ تعديل وحذف لتسجيل القطع · ✍️ الخط

**الطلب:** «الإشعارات لسه تروح للطالب لمن الطلب ينتقل من مرحلة لمرحلة — وقّفها هسة · ضيف
حذف وتعديل لتسجيل قطع عمال الورشة · مصطفى علي عنده وشاح بس وهو عنده وشاح وروب، وين روبه؟ ·
والمدير كَال الخط زين بس مو زين — افحصه فحص حقيقي.»

### ١. 🔕 إشعارات المراحل وقفت (`backend/lib/studentOrderNotices.js`، جديد)

ثلاث أماكن كانت تكتب «حالة طلبك الآن: ${'{'}STATUS_LABEL_AR[to]{'}'}» للطالب:
`orderController.updateStatus` · `productionController.performAdvance` · `.revert`.
غلطتين بنفس الوقت:
- **ضجيج.** الطقم عدة سطور بـ`orders`، فحركة وحدة تطلّع إشعار **لكل قطعة**. و`updateStatus`
  ما عدها حتى شرط `prev !== status` — إعادة حفظ طلب على نفس مرحلته كانت تكتب إشعار عن حركة
  ما صارت. هذا حرفياً «stage x moved to stage x» اللي انذكر.
- **تسريب مفردة داخلية**، بعد شغل «قيد التنفيذ» بنفس اليوم ما خلّص إلا وهي ترجع من باب ثاني.

الوقف **flag مو حذف** (`STAGE_NOTICES_ENABLED = false`)، والدالة اللي وراه تنطي **مفردة
الزبون** مو `STATUS_LABEL_AR` — يعني تشغيلها مرة ثانية ما يكدر يسرّب مرحلة مهما كبر الـenum.
باقية على حالها: «تم تسليم طلبك» · «تم إرجاع طلبك للتعديل» · موافقة/رفض التصميم · موافقة الممثل.

### ١ب. 🔔 وبدالها: «طلبك جاهز للاستلام» — إشعار واحد للطقم كله

`notifySetReady` بنفس الملف + **migration 111** (`orders.ready_notified_at`). نفس قاعدة
`MyOrdersList.tsx` حرفاً: المجموعة `checkout_group_id` (وإذا ماكو فالطلب نفسه)، وما تنكَال
«جاهز» إلا لمن **كل** قطعة حيّة توصل ready/delivered. إشعار لكل قطعة كان راح يكون نفس الغلط
اللي انصلّح بالشاشة (١٨٠ طالب بحالة مختلطة) — بس هالمرة يوصل التلفون وما ينرد.

⚠️ **وتنشغل على كل تغيير حالة مو بس على `ready`، لأنها بعد الـre-arm.** ثلاث حالات تخلّي
طقم انعلن عنه يرجع ناقص: رجعة قطعة ورا من ready، وإعادة إرسال قطعة مُرجَعة، وقطعة جديدة
تدخل المجموعة. بالثلاثة انمسح الختم، وأول ما يخلص من جديد ينعلن مرة ثانية — وهذا الصح:
الطالب إجا وما لكاه، انسحب، وهسة فعلاً جاهز. القاعدة بمكان واحد بدل ثلاث كتّاب لازم
يتفقون للأبد.
⚠️ **القطعة المُرجَعة مو من المجموعة** — `myOrders` يفلترها، فالطالب أصلاً ما يشوفها بـ«طلباتي».
⚠️ **الـbackfill مالت 111 داخل فرع `IF NOT EXISTS` بقصد**: `schema.sql` ينطبّق كل نشرة، و
`UPDATE ... WHERE ready_notified_at IS NULL AND status IN ('ready','delivered')` يقرا كأنه
idempotent وبالحقيقة **يبلع** إشعار أي طقم خلص بين نشرتين. نفس فخ ٢٠٢٦-٠٩-١٢. وبدون backfill
أصلاً، أول طقم يخلص بعد النشر يسحب وياه كل طالب طلبه جاهز من زمان (**٧٠٦ طلب** بالنسخة
المحلية). ٧ تستات بـ`test/setReadyNotice.test.js`.

### ٢. ✏️ تعديل وحذف لتسجيل القطع (اللي انطلب فعلاً)

`PATCH /workshop/production/:id` و`DELETE /workshop/production/:id`.
⚠️ **ماكو `requireLead` على الراوت — الحارس جوّا (`loadEntryFor`)**، لأن العامل العادي
يعدّل ويحذف **سطره هو** وبس. هذا ما ينطيه صلاحية جديدة: أصلاً يكدر يسجّل أي شي لنفسه عبر
`myProduction`، فالباب هذا يشيل رقم غلط من السجل مو أكثر. سطر غيره → 403.
⚠️ **الأجر ينحسب من `workshop_piece_rates` دائماً** — لا `rate` ولا `amount` ينقبلون من
العميل، نفس مسار الإدخال. والتحقق يصير **بعد** دمج التعديل الجزئي، لأن (القطعة، الشغل،
لمين) ثلاثي وبس الثلاثي إله سعر. الحذف نهائي + سطر `audit_log` يحمل المبلغ كامل.
الواجهة: مكوّن واحد مشترك `components/workshop/ProductionEntryActions.tsx` بين شاشة العامل
`/workshop` («آخر الحركات») ولوحة الإدارة `/admin/workshop` («آخر التسجيلات»). الحافز والخصم
ما إلهم أزرار — قرار إداري مو تصحيح تسجيل.

**🐛 وانكشف خلل حقيقي وقت الفحص بالمتصفح:** `work_date`/`entry_date` أعمدة `date`، و`pg`
يرجّعهن كـ`Date` على منتصف ليل **السيرفر**، و`JSON.stringify` يكتبها UTC — فـ٢٠٢٦-٠٩-١١
كانت تطلع `"2026-09-10T21:00:00.000Z"`. لوحة الإدارة كانت تطبع هالنص حرفياً، ومودال التعديل
الجديد يقرا أول عشر حروف → **كل حفظ يرجّع تاريخ الشغل يوم ورا**. الحل `to_char(...,'YYYY-MM-DD')`
بالاستعلام — نفس اللي يسويه `salaryController` من الأول. نفس فخ `attendanceController.dateKey`
(٢٠٢٦-٠٩-٠٨). الحارس `test/workshopEntryDate.test.js` و**أول سطر بيه** `process.env.TZ`.

**زيادة على الطلب (قبل ما يتوضّح المقصود):** انضاف بعد «تعديل/حذف» لسطر **العامل** نفسه
بتبويب «العمّال». الحذف يرفض (409) أي عامل عنده سجل أجور — `workshop_production_entries`
و`workshop_adjustments` CASCADE وينمسح سجله وياه — ويقول «استخدم إيقاف». وما يمس `users`
أبداً (الحذف يكاسكيد على البصمات والراتب): حساب ورشة يتعطّل بـ`deleted_at`، وحساب موظف
مرتبط ما ينلمس. إذا مو مطلوب، ينشال بسهولة.

### ٣. 🔎 وين روب مصطفى علي؟ — ماكو ضياع

مصطفى علي (`4e96e6e8`) عنده **ثلاث** طلبات من ٢٠٢٦-٠٦-٣٠: وشاح ملكي · قبعة ملكة · روب فصال.
بـ٢٠٢٦-٠٧-٠١ المدير ضغط «إرجاع للطالب» على **الثلاثة** بسبب «عدل ع طلب». الطالب أعاد إرسال
**الوشاح بس** — فرجع للإنتاج ووصل التطريز. الروب والقبعة لسه `returned_to_customer = TRUE`
من ذاك اليوم، والقبعة انرجعت مرة ثانية ٠٧-٠٢ («ضيف صورة قبعة»).
`orderController` يخفي المُرجَع من «طلباتي» ومن قوائم الإنتاج، فما يظهر إلا بـ`/returned-orders`.
⚠️ **والمقاس يكَول إنها مو حالة فردية: ٣٢ طلب مُرجَع عند ١٦ طالب، كلهم تجزئة، وكلهم
مرّ عليهم أكثر من ٣٠ يوم** — أقدم ٠٦-٣٠ وأحدث ٠٧-٢٤، وولا واحد انعاد إرساله. الطالب ينطى
إشعار مرة وحدة يوم الإرجاع وبعدها ماكو شي يذكّره. (الروب هذا حتى خلّصه ابو عبدو `tailor_complete`
بـ٠٨-٢٤ وهو مخفي من كل شاشة.) قرار المالك: نذكّرهم؟ نتصل بيهم؟ نعرض المُرجَع بـ«طلباتي»؟

### ٤. ✍️ الخط: السبب كلمة وحدة بالـprompt

فحص حي على `google/gemini-3.1-flash-image` بنفس الخمس أسماء بنفس اليوم:
- **بالصيغة القديمة — ٠ من ٥ نظيفة.** `BASE` كانت تطلب حرفياً **«masterful diacritics»**،
  فرجعت كل الأسماء مشكّلة («مُصَطَفَى عَلِي»، «دِيَّالَى» بشدّة ما إلها محل بالكلمة أصلاً)،
  والتشكيل جرّ وراه **حروف زايدة**: «ص» طايرة تحت مصطفى، «ء» وسط علي، «مُ» زيادة جنب حوم،
  «س» ثانية تحت قسم. الموديل يرسم العلامات بوزن الحرف، فعلى الوشاح تصير غرز ما طلبها أحد.
- **بدونها — ٥ من ٥ نظيفة** ومكتوبة صح وبدون تشكيل. نفس الموديل، نفس السعر، نفس ١٦ ثانية.
- ⚠️ **وشيل طلب التشكيل شال وياه ضغط القلم** — أول ورقة بدون تشكيل رجعت رفيعة، وهذا للتطريز
  أسوأ مما يبيّن بالشاشة. فانضافت صيغة «قلم عريض بضغط قوي، ما تكون رفيعة». **التغييرين
  يمشون سوا ويرجعون سوا.**
- الورقة الجديدة تنقصّ ٥/٥ عبر `lib/sheetCrop.js` مثل القديمة بالضبط. الكلفة $0.101/ورقة.
⚠️ باقي خلل **قديم** ما إله علاقة: القص يكصّ رؤوس الحروف الطويلة (ط/ل/ا) من فوق — موجود
بالقديم والجديد سوا.

**الفحص:** ٧٧٨ تست بالباك — ٧٧٥ ناجحة، فشل ٢، والاثنين موجودين على `main` النظيف قبل
الشغل هذا: `app-open` المتذبذب و`empty marketing audience` (انفحصوا بـ`git stash`).
⚠️ و`assemblyBoard`'s «a rep sash: arriving…» **متذبذب من قبلنا**: يقارن ترتيب مفاتيح
`embroidery_zones` بـ`deepEqual` بدون ترتيب — فشل ٢ من ٤ تشغيلات على `main` النظيف · `tsc` و`eslint` نظاف · الشاشتين انفتحن
بمتصفح حقيقي: تعديل حقيقي (١٠ → ٣ قطع، ١٠٬٠٠٠ → ٣٬٠٠٠ د.ع) وحذف حقيقي من حساب **عامل**،
والصفين انرجعوا للـdev DB بعدها. ⚠️ **عرض الهاتف ما انشاف بالعين** (نفس عائق الشاشة
العريضة من ٠٨-٢١) — بس انقاس: ماكو تمدد أفقي، الأزرار ٤٤px، والسطر يلتف لسطرين بـ٣٥٨px.

---

## 2026-09-14 — 🧹 زر «فرّغ الرف»، و«جاهز للاستلام» صارت تكَول الصدق

**الطلب:** «زر يفرّغ رف التجهيز مع رسالة تأكيد · وأكو خلل: طلاب هوايه يشوفون طلبهم جاهز
للاستلام وهو مو جاهز · وأريد أعرض معلومات أقل بهذي النقطة، لا تعرض كل المراحل.»

### ١. الخلل ما كان بالكود — كان بوحدة القياس

ما صار ولا تقديم غلط. الموظفين فعلاً حرّكوا القطع (٢٥٤ قطعة يوم ٩-٩، ٢٢٨ يوم ٩-١٠). المشكلة
إن شاشة «طلباتي» كانت تطبع حالة **كل قطعة لحالها**: طالب خلصت قبعته ووشاحه لسه بالكوي كان
يقرا «جاهز للاستلام» جنب «قيد الكوي»، فيجي للمحل وما أكو شي يتسلّمه.

**المقاس على نسخة الإنتاج المحلية:** **١٨٠ طالب** عندهم قطعة بـ`ready` وقطع ثانية لسه
بالإنتاج — و**١٧٦** منهم القطع المختلطة بنفس `checkout_group_id`. هذا اللي يخلي التجميع على
هالعمود **حل**، مو تخمين. بعد التصليح: ١٠٦ طالب ينكَلهم «جاهز»، وكلهم طلبهم كامل فعلاً.

**شنو تغيّر (`frontend/components/student/MyOrdersList.tsx` — أعيدت كتابتها):**
- الطلب صار **مجموعة** (`checkout_group_id`، وإذا ماكو فالطلب نفسه). و«جاهز للاستلام» ما
  تنكَال إلا لمن **كل** قطعة حيّة بالمجموعة توصل ready/delivered.
- **ولا مرحلة إنتاج توصل للطالب.** التصميم · التطريز · التجميع · الكوي · التجهيز هذي مفردات
  المحل مو مفردات الزبون. صارت ثلاث حالات بس: «قيد التنفيذ» · «جاهز للاستلام» · «تم التسليم»
  (وياهن «بانتظار الموافقة» و«ملغي»). القطعة الملغية تنشطب لحالها وما تمنع باقي الطلب.
- سطر «أُعيد طلبك للتعديل» بقى — لأنه الشي الوحيد اللي يكدر الطالب يسوي بيه شي.

**ونفس الخلل كان بـ«لولو»** (`backend/lib/supportContext.js`): كانت تنطي الموديل
«الحالة: جاهز للاستلام» على قطعة وحدة. صارت تنطيه «خلصت هاي القطعة» للقطعة، وسطر واحد على
مستوى الطلب كله يكَول جاهز أو «ما يجي يستلم». ⚠️ هذا **عكس** قاعدة قديمة بالتست («حالة غير
معروفة تنطبع خام») — وانعكست بقصد: الخام هو بعد مفردة داخلية.

### ٢. «فرّغ الرف» — يمسح الخريطة، ما يسلّم ولا قطعة

`backend/lib/shelf.js → clearShelf()` · `POST /api/production/shelf/clear` ·
زر بعنوان قسم «الرف» مع `Modal` تأكيد (مو `window.confirm` — يجمّد الـWebView).

⚠️ **القراءة البديهية لـ«فرّغ الرف» هي «سلّم كل شي»، وهذا بالضبط اللي ممنوع.** لو انكتب
`collectForOrder` بحلقة، جان صار ٤٨٠+ قطعة «جاهز للاستلام» بضغطة — أي نفس الشكوى بس أكبر.
فهو `DELETE` للتسكينات الحيّة + إغلاق الخانات الفارغة، **بلا أي تغيير بـ`orders.status`**.
التسكينات المجموعة سابقاً ما تنلمس (منها ينقرا «منو غلّفها»). صلاحية **مدير/أدمن بس**
(`requireStaffType()` بلا أنواع)، لأن المجهّز عنده «إرجاع» لكل قطعة للغلطة العادية.

نص التأكيد يكَول **شنو ما يسويه** بصراحة: «القطع ما تنمسح وما تتسلّم» و«ولا طلب راح يصير
جاهز للاستلام بهذا الزر»، ويعرض عدد القطع اللي على الرف هسه.

### ٣. التحقق

- تست جديد بآخر `test/shelf.test.js` — يصوّر الرف، يفرّغه، يتأكد **ولا حالة طلب تغيّرت**، ثم
  يرجّعه ويتأكد من الرجعة.
  ⚠️ **كتبته أول مرة بملف لحاله (`shelfClear.test.js`) وهذا كان غلط ويكسر السويت.**
  `node --test` يشغّل **الملفات** بالتوازي، فملف يمسح الرف كله سحب الأرض من تحت تستات
  التسكين بنفس اللحظة — والضرر بقى بعد الشوط: **٢٠ خانة انسدّت (`closed_at`) وهي لسه بيها
  قطع حيّة**، وهذا يكسر أهم شرط بالرف. انصلحت القاعدة بالإيد، والتست انتقل جوّا
  `shelf.test.js` (آخر شي) حتى ينمشي بالتسلسل، وصار يتأكد بنفسه من الشرط بعد الإرجاع.
- ⚠️ **وهذا الخراب كان يغطي على تستين هشّين، وهسه انصلحن.** لمن رجّعت القاعدة لحالتها
  الصحيحة طلع `shelf.test.js` عنده تستين يفشلون **حتى على `main` النظيف** (مثبّت بـ`git stash`)
  لأنهن يفترضون إن رف الشال فاضي: وحدة تكَارن `bin.count === 2` (صارت تكَارن **الفرق** مو
  الرقم المطلق)، والثانية تحشر خانة بـC07 فتصطدم بخانة شال حقيقية مفتوحة بـC11 عبر
  `shelf_slot_one_open` (صارت **تركن** أي خانة C حيّة وترجّعها بالـ`finally`، وما تمسحها أبداً).
- `test/aiChat.test.js` — تست جديد يثبّت «جاهز للاستلام للمجموعة مو للقطعة»، وتست قديم
  انكتب من جديد.
- **بالمتصفح، بضغطات حقيقية:** الزر ← تأكيد ← **١٧٤ تسكين و٢ خانة صاروا صفر، ٤٨٠ تسكين
  مجموع ما انلمس، وصفر تغيير بالحالات**، والقطع رجعن لقائمة «وصلت توّا» (٤٧٨ → ٦٥٢). و«إلغاء»
  ما تسوي شي. وشاشة الطالب: طالبة عدها ٣ قطع جاهزة وقبعة بالتجهيز شافت «قيد التنفيذ» على
  الأربعة، وطالبة كل قطعها جاهزة شافت «✓ طلبك جاهز للاستلام». مقيسة بعرض ٣٩٠px بلا تمدد أفقي.
- `tsc` + `eslint` نظاف. الباك: **٧٥٩ تست، ٣ فشل موجودة أصلاً على `main` النظيف**
  (`appPresence` و`pushBroadcast` بسبب صفوف تجريب قديمة بقاعدة اللابتوب، و`adminNumbers`
  هو سباق الـCOUNT المعروف) — ولا وحدة منهن من هذا الشغل.

**⚠️ الشغل مو مرفوع ومو مدموج — `main` تنشر تلقائياً عند الدفع.**

## 2026-09-12 (b) — 📊 «الإحصائيات»: شاشة وحدة تكول هل نكبر، منو يفتح التطبيق، ووين يقضون وقتهم

**الطلب:** «قبل ما نقفل المتصفح على المتصفحات — أكدر أشوف تحليل كامل؟ منو يفتح ومنو داخل
بحساب، شكد تنزيل، وشكد نكبر، والطلاب بأي صفحة يكعدون؟» وبعدها «أشوفها بشاشة المدير بالتلفون».

**شنو انبنى:**
- `backend/lib/shopAnalytics.js` + `GET /api/admin/usage` — كل شي بطلب واحد.
- `frontend/app/admin/analytics/page.tsx` + رابط «الإحصائيات» بقائمة المدير. مبنية للتلفون
  أولاً: كل شبكة تبدي عمودين، وما بيها جدول يتسحب بالعرض.
- `test/shopAnalytics.test.js` — ٥/٥ ناجحة.

**⚠️ القاعدة اللي تحكم الصفحة كلها: أربع مصادر تقيس أربع أشياء مختلفة، وما ننجمعهن أبداً.**
`users` (حسابات، من يوم فتح المحل) · `app_opens` (استعمال، **بس اللي داخل بحساب**، ومن
2026-08-25 فقط) · `site_visits` (زيارات تشمل بلا حساب، بيها **المسار** بس **بلا منصة**،
وبس على صفحات الطلاب) · `device_tokens` (**أرضية** للتنزيلات: تنزيل + دخول + موافقة على
الإشعارات). كل قسم بالصفحة يكتب من أي يوم بدأ القياس مالته، لأن جدول جديد بلا هذا السطر
يقرا وكأن «محد يستعمل التطبيق».

**الأرقام يوم البناء** (من نسخة الإنتاج المحلية `loloshop_prod` على :5433، مسحوبة نفس اليوم — ⚠️ **مو** قاعدة `loloshop` اللي على نفس السيرفر وواقفة على ٣١ آب؛ الخلط بينهن كلّف نص جلسة): ٢٬٦٤٩ حساب طالب · اليوم ٢٤٨ زيارة و١٦٨ شخص داخل بحساب و٢٢١
فتحة · فتحوا التطبيق ولو مرة ١٬٠٣٠ طالب (٣٩٪)، منهم ٧١٧ على التطبيق نفسه (٢٧٪) · الأجهزة
٥٢٨ أندرويد + ٣٨٠ آيفون (٢٩٣ منها بلا حساب) · الصفحات: الرئيسية ٤٬٧٩٤ زيارة (~١٩ دقيقة)،
صفحة منتج ١٬١٦٧ (~٧)، طلبي ٢٧٧ (~١٧).

**الفجوات: وحدة باقية وثنتين انسدّن بنفس الجلسة.**
1. **باقية — عدد التنزيلات الحقيقي ما موجود بأي جدول**، بس بحساب Google Play و App Store.
   `device_tokens` أرضية وبس.
2. ✅ **انسدّت — `platform` على `site_visits` (ترحيل ١١٠).** هذي المكان الوحيد اللي يشوف
   منصة زائر **بلا حساب** (`app_opens.user_id` هو NOT NULL بالتصميم)، يعني هي كشّاف
   البوابة: بعد «التطبيق فقط»، الفرق بين زيارات التطبيق وزيارات المتصفح هو الدليل
   الوحيد إنها اشتغلت. ⚠️ **NULL يعني «قبل ١١٠» وما يعني `web`** — ٣٤ ألف صف قديم لازم
   يبقون بخانة «غير معروف»، وإلا صار عدنا ادعاء محد قاسه. المنصة **يدّعيها العميل** وما
   تنفع تحكم بشي؛ البوابة تقرر من `window.Capacitor` بالمتصفح.
3. ✅ **انسدّت — `VisitBeacon` انتقل للـroot layout ويشتغل على كل تنقّل.** قبلها كان بس
   بـ`app/(student)/layout.tsx`، يعني `/get-app` و`/login` و`/join` ما ينقاسون أبداً —
   وهي الصفحة اللي راح توصلها كل زيارة متصفح بعد البوابة. ⚠️ **قائمة الاستثناء داخل
   الكومبوننت هي اللي تخلي النقلة آمنة:** `/admin` و`/staff` والورشة والتلفزيون مفتوحات
   طول اليوم على أجهزة المحل، ولو انحسبن جان دخل شغل الموظفين بـ«الزيارات الآن» ودفن
   ترتيب صفحات الطلاب. `/wholesaler` **مو** مستثنى عمداً.

**⚠️ سباق حقيقي انكشف أثناء الفحص وانسدّ:** dedup الخمس دقائق بالسيرفر
(`INSERT … WHERE NOT EXISTS`) **مو ذرّي** — تحت READ COMMITTED پنگتين بنفس الميلي ثانية
ثنتيهن ما يشوفون صف ويكتبون. مقاس: جلسة وحدة، مسار واحد، **صفّين بفرق ١ ملي ثانية**. كان
ممكن من زمان، بس صار مرجّح لمّا الـbeacon صار يشتغل على كل تنقّل. الصف الزائد مو تجميلي:
الصفحة تقرا الصف كـ~٥ دقائق، فالتكرار **ينفخ «دقيقة للزيارة»**. الحل حارس ٦٠ ثانية
لنفس المسار **بمستوى الموديول** (مو `useRef`، لأن لازم ينجو من إعادة التركيب ومن
الاستدعاء المزدوج بالتطوير) — نفس نمط `AppBeacon`. تأكّد بالمتصفح: صف واحد بعد الإصلاح.

**الفحص بالمتصفح (على نسخة الداتابيس المحلية، مو على الحية):** `/get-app` ينكتب
`platform=web` ✅ · `/admin` **ما ينكتب** ✅ · `/login` ينكتب — صفحة ما كانت تنقاس أبداً
قبل اليوم ✅ · المتجر لسه ينقاس بعد النقلة ✅ · `by_platform` يطلع ٨٬٩٣٠ جلسة بـ«غير
معروف» و«web» لحالها، يعني القديم ما انطوى بالجديد ✅.

**البوابة نفسها:** الكود منشور على prod (`bd61322`) و`/get-app` ترجع 200، بس
`NEXT_PUBLIC_APP_ONLY` مو مضبوط بـ`frontend/.env.local` — ولأنه `NEXT_PUBLIC_*` ينحط وقت
البناء، تشغيلها = تعديل الملف + `npm run build` + restart، مو `--update-env`.


## 2026-09-12 — 🧨 الديپلوي نفسه كان يرجّع القطع المكوية للكوي، وسجل القطعة صار سجل شغل

**الشكوى:** «هيام صارت تم الكوي، بعدين اتسكنت بالتجهيز، وفجأة رجعت للكوي» · «ما يظهر منو
نطاها تم ولا منو رجّعها، وأصلاً محد رجّع» · «أكو قطع ما تتسكن صح» · «أكو خلل ببحث التسكين».

**السبب الجذري — مو أحد، الديپلوي.** نسخة الترحيل 101 اللي داخل `db/schema.sql` كانت
منقسمة لثلاث جمل، وآخر وحدة تشتغل على جدول `legacy_pressing_restore_log` **كله** (307 صف
ثابتين) بشرط وحيد `o.status='preparing'`:

```sql
UPDATE orders SET status='pressing' FROM legacy_pressing_restore_log l
 WHERE o.id = l.order_id AND o.status = 'preparing';
```

و`scripts/deploy.sh` يطبّق الملف **بكل ديپلوي**. يعني: المكوجي يكوي ← المجهّز يسكّن ←
أي ديپلوي يرجّعها للكوي. وبصمت تام: سطر `route_fix` فوكَه محمي بـ
`NOT EXISTS(action='route_fix')` فما ينكتب ثاني مرة، ولا ينكتب صف `audit_log`.

**الدليل:** مجاميع `orders.updated_at` **بنفس المايكروثانية** بعد كل ديپلوي بدقيقة —
11 صف 09-07 23:51 · 6 صفوف 09-08 16:55 · 14 صف 09-08 23:29 — مطابقة لأوقات `1e61191`
و`cd64150` و`8b69c22`. القياس: **49 طلب** واقف بالكوي رغم إن بشر كواه، **46 منهن
محطوطين بخانة على الرف**. ملف الترحيل نفسه كان سليم دائماً؛ النسخة هي اللي انكسرت.

- `db/schema.sql`: 101 صار بلوك DO واحد مربوط بـ**دفعة تشغيله**. متحقَّق: صفر صف يتحرك
  على بيانات الإنتاج الحالية.
- `db/migrations/109`: يرجّع الـ49، ويغلق 14 تسكين ميت. **متعمّد ما ينكرر بـschema.sql** —
  جملة متكررة هي اللي سببت المشكلة.

**سجل المراحل صار سجل شغل.** كان يقرا `staff_activity_log` بس، وكل هذا كان مخزون
بـ`audit_log` وما يظهر بأي شاشة: **1,052** تطريز منطقة · **482** إنهاء فصال · **147**
اعتماد/رفض تصميم · **96** إرجاع للطالب · **474** نقلة سكربت `stranded-orders` (268 طلب
تجزئة وقفوا بالكوي وبطاقتهم فارغة) · **100** تعديل موظف رجّع الطلب لـ«بانتظار التصميم»
(39 من التطريز، 32 من التجهيز، 29 من الكوي). الحين البطاقة تقرا المصدرين مع منع التكرار،
وتكتب فعلاً بالعربي («طرّز الوشاح — جهة الاسم»، «أنهى الفصال»، «عدّل الطلب ورجّعه: …»)،
وفوكَها سطر «اشتغل عليها: …». «تصحيح مسار آلي» بلا اسم — بس للحركة اللي فعلاً محد سوّاها.
التغطية: طلبات التجزئة بلا تفسير **129 → 0**.

**«أكو قطع ما تتسكن صح».** `revert` يحرّر الخانة من يوم ما انبنى الرف، و`performAdvance`
لا — فالمجهّز اللي يضغط «جاهز» من غير «تسليم» مال الرف يخلّي التسكين مفتوح. 14 حالة.
الأمام يصير **collected** مو delete (القطعة انلمّت فعلاً، وحذف الصف يمحي منو غلّفها).

**«أكو خلل ببحث التسكين» + القبعات (قرار المالك: «القبعات لازم تتسكن»).** القبعة السادة
**تنفتح** على التجهيز وما تنتقل له، و`buildBoard` كان يطلب صف `to_stage='preparing'` —
فما شافت ولا قبعة أبداً: **483 قبعة** واقفة، ولا وحدة متسكّنة، القسم C01–C10 فارغ من يوم
ما اشتغل الرف، و**214** منهن ما توصلهن حتى خانة البحث. صار الانفتاح على التجهيز = وصول
(بنفس حدّ `shelf_epoch`)، وانضاف فلتر نوع القطعة حتى 423 قبعة ما تدفن 43 وشاح.

**⚠️ تحذيرات أمنية جديدة وقّفت الديپلوي** (`npm audit` بوّابة داخل CI): **next 16.2.11 →
16.3.5** (`GHSA-2xp9-vwfh-vxw4`، تنفيذ أوامر عن بُعد بلا مصادقة عبر Image Optimization API
مع AVIF — **حرج**) · **sharp → 0.35.4** · **multer 2.2.0 → 2.3.0**. قفل فقط، بلا تغيير
`package.json`. و**Next 16.3 حذف `experimental.viewTransition`** فصار خطأ أنواع يفشّل
البناء — انحذف، والبديل `<ViewTransition>` من React يحتاج **يلف** العنصر، يعني 4 مكونات
تحتاج تحويل صغير حتى ترجع حركة صورة المنتج. تجميلي، مو وظيفي.

**الفحص:** `node --test test/*.test.js` → 741/749 (السبعة الفاشلة نفسها تفشل على `main`
نظيف مقابل نفس نسخة البيانات — تعتمد على شكل البيانات أو ترتيب التشغيل) · 3 ملفات اختبار
جديدة/موسّعة كلها أحمر ثم أخضر · `tsc` · `npm run lint` · `next build` · `npm audit` نظيف
بالاثنين.

**باقي مفتوح:**
- تشغيل `npm run migrate:file db/migrations/109_undo_deploy_pressing_revert.sql` على الإنتاج
  بعد نزول الديپلوي. النسخة الاحتياطية قبل النشر:
  `/root/loloshop-prod-2026-09-12-predeploy.dump` (وعلى اللابتوب بـ`~/Desktop/_private/loloshop-db/`).
- تحويل الـ4 مكونات لـ`<ViewTransition>` حتى ترجع حركة صورة المنتج.
- `designTeamController.finishJob` ينقل design_complete → embroidery ويكتب `approve_design`
  بلا صف نشاط — الاسم يظهر بالبطاقة، بس رواتب/أهداف الموظفين ما تحسبها (تعدّ
  `action IN ('advance','approve_design')` من `staff_activity_log`).

## 2026-09-08 (c) — 🔍 «إنهاء الكوي» يظهر ثم يُرفض — انلكَت بالبيانات، مو بالنص

النص «يرجى مراجعة الطلب» **مو موجود بالمستودع ولا بحزمة الإنتاج** — كان وصف الأدمن، مو
اقتباس. اللي لكَه هو **استعلام على الإنتاج**، بعد ما التلميح الوحيد المتبقي كان «الخلل على
طلاب التجزئة»:

```
 retail | pressing  | returned_to_customer = t |  11
 retail | preparing | returned_to_customer = t |   3
 ...وصفر طلب ممثل يحمل العلم بأي مرحلة إنتاج
```

`advanceBlockReason` يرفض هيك طلب بـ**«الطلب مُرجَع للطالب»** (409)، بس
`available_actions.advance` كان يسأل `canStaffTransition` بس — وهذا يعرف **الأدوار** ولا يعرف
شي عن حالة الصف. فالصفحة تعرض «إنهاء الكوي، نقل للتجهيز»، والضغطة ترجع ٤٠٩، والموظف ما عنده
شي يسويه لأن **الطالب** بس يكدر يعيد الإرسال.

`returned_to_customer` عملياً شي تجزئة (الطالب يعدّل بـ`/returned-orders`)، ولهذا التقرير
وصل «على طلاب التجزئة».

**الطابور يخفي هذي الصفوف أصلاً** (`getQueue` فيه `AND o.returned_to_customer = FALSE`)،
فمدخل الحالة هو **صفحة التفاصيل**: رابط من `/admin/orders`، أو تبويب قديم.

**نفس شكل العطل اللي مكتوب بعنوان `embroideryChecklistBlocks` بالضبط، بس بوابة أبعد:** منحة
محسوبة من الدور، ورفض محسوب من الصف، والاثنان ما يتفقون.

### الحل
`available_actions.advance` صار `null` لما `advanceBlockReason` يرفض، وانضاف
**`advance_block: { code, reason, message }`** حتى الشاشة تكدر تكَول ليش. بالواجهة
`AdvanceBlockNotice` يعرض رسالة السيرفر + سطر «شنو الخطوة الجاية»
(«الطالب يعدّل طلبه ويعيد إرساله» / «الممثل لازم يوافق أول»).

⚠️ **إخفاء الزر مو البوابة.** `advance` و`advanceBulk` و`sendOrder` كلهم لسه ينادون
`advanceBlockReason` ولسه يرجعون ٤٠٩ — إخفاء زر ما يوقف id منشور باليد، وهذا نص لغم
«بانتظار موافقة الممثل». الفحص يثبت **النصفين سوة**، عمداً.

⚠️ `delete order.wholesaler_id` لازم يجي **بعد** قراءة الرفض. `advanceBlockReason` يقرأ
`order.wholesaler_id != null`، و`undefined` يمرّ — فحذفه بسطر واحد أبكر يقتل نص الممثل بصمت.
مثبّت بفحص مستقل.

**فحوص:** `test/advanceBlockSurfaced.test.js` (٦، كلها كانت حمراء) · `tsc` + `eslint` نظاف ·
**744 فحص، ٣ فاشلة وكلها تفشل على `main` النظيف** (`noteGroupRouting`، `pushBroadcast`،
وفلاكي `adminConsole`/`allocatePin` من التوازي — كل ملفاتي تنجح لوحدها).

---

## 2026-09-08 (b) — 👁️ محمد عادل (المكوجي) يشوف الطلب كامل ما عدا الفلوس + «مراجعة الطلب» ما انلكت

نفس الفرع `fix/attendance-break-visibility`. لا هجرة، لا تبعية.

### اللي تغير — قرار المالك «everything except money»

`getOrder` كان يعطي المكوجي (لما هذا دوره **الوحيد**) شاشة مصغّرة: الاسم + صورة المنتج +
القياسات + «شريحة» تصميم فيها اللون بس. قرار 2026-07-15، والمالك ألغاه اليوم — محمد عادل هو
اللي يمسك الطقم كله بيده، وإخفاء التصميم وجهة الاتصال وبقية القطع كلفه سياق وكلفت المحل
مكالمات. **الفلوس ما تحركت أبداً.**

| | قبل | بعد |
|---|---|---|
| الكانفس الكامل للتصميم | ✗ (لون فقط) | ✅ |
| هاتف الطالب + انستغرام | ✗ | ✅ |
| بيانات الطلبية (انستا) | `event_date` فقط | ✅ كاملة (بلا عربون) |
| التوصيل (اسم/عنوان/هاتف المستلم) | ✗ | ✅ |
| بقية قطع الطقم | ✗ | ✅ |
| **السعر / العربون / سعر السطر** | ✗ | **✗ — ما تغير** |

`view.layout` ما عاد يرجّع `'presser'` إطلاقاً؛ يوقع على الصفحة الكاملة، و**اللي يشوفه**
يتقرر بالخوادم (شيل الفلوس) و**اللي يسويه** بـ`available_actions` (تعديل/حذف للمدير فقط) —
مو باللايوت. انحذفت شاشة المكوجي المصغّرة من الواجهة (١٧٠ سطر ميت).

⚠️ **الفحص لكَط تسريب فلوس حقيقي بنفس اللحظة:** «الباقة الكاملة» كانت ترجّع `price` لكل قطعة
شقيقة. كانت آمنة بالصدفة لأن `canSeePackage` و`canSeeMoney` كانوا نفس المجموعة (الاستقبال
والمدير)؛ أول ما دخل المكوجي للباقة صار تسريب على شاشة تخفي `order.price` بسطرين فوقه.
هسة السعر الشقيق مربوط بـ**`canSeeMoney`**، مو `canSeePackage`. مثبّت بفحص.
· وزر «نسخ بيانات الطلبية (انستا)» صار خلف `showPrice` — نصّه **هو** ملخص مالي
  (المجموع + العربون + المتبقي).

**مو تغيّر:** الفصال والمطرّز يبقون على قوائم السماح مالتهم بلا مساس.

**فحوص:** `test/presserOrderDetail.test.js` (٣، كانت حمراء) · `tsc` + `eslint` نظاف ·
**مسيَّر بمتصفح حقيقي** كـمحمد عادل على طلب تجزئة: الصفحة تعرض سجل المراحل + الهاتف + انستغرام
+ الجامعة + خيارات الطلب + الباقة (وشاح · روب · قبعة) و**صفر «د.ع» بكل الصفحة**.

### «يرجى مراجعة الطلب» — ما انلكَت، والبحث موثّق

النص **مو موجود بالمستودع ولا بحزمة الإنتاج المبنية**. انبحث عن «مراجعة» و«راجع» بكل
الملفات وبـ`/var/www/loloshop/frontend/.next` على الصندوق — أقرب شي موجود:
«بانتظار المراجعة» (شارة موافقة التصميم) · «قيد المراجعة» (`staff_review`، ما بيه ولا طلب
حي) · «يرجى تعديل الطلب وإعادة إرساله» (إشعار إرجاع الطلب للطالب) · «لا يمكن إكمال هذه
القطعة من هنا حالياً» (`StudentSheet` لما `canComplete` كاذبة).

**انعاد بناء الحالة بمتصفح حقيقي مرتين** على طلبات **تجزئة** كـمحمد عادل، والاثنان اشتغلوا
صح بلا أي رسالة:
· من «عرض بالطلب» → «إنهاء الكوي، نقل للتجهيز» → توست «انتقلت إلى التجهيز ✓» ثم ورقة الرف
  «أين نضع روب؟ ضعها في الخانة A01».
· من صفحة «التفاصيل» → نفس الزر → انتقل وردّ على `/staff` بلا خطأ.

**نتيجة جانبية حقيقية (ما انصلحت — تحتاج قرار):** التقدم من **صفحة التفاصيل** ما ينده
`offerShelfPlacement` أبداً — هذا موجود بـ`StationConsole.completePiece` فقط. فقطعة تجزئة
انكملت من «التفاصيل» توصل التجهيز بلا خانة. ما تضيع (تظهر بصندوق وارد «رف التجهيز»)، بس ما
تنسأل عنها.
· **مقاس على الإنتاج:** 457 **قبعة** تجزئة بالتجهيز و**كلها** بلا خانة، ورف القبعة (C،
  ١٠ خانات) فاضي تماماً — لأن القبعة قطعة سادة تبدأ **بالتجهيز** فما تمر بالكوي أصلاً.
  وشاح ٧ · روب ٧ · شال ٥ بلا خانة، وهذي على الأغلب اللي انكملت من صفحة التفاصيل.

**الباقي محتاج لقطة شاشة من الأدمن** — بلا نصّها الحرفي أي إصلاح راح يكون تخمين.

---

## 2026-09-08 — 🔍 «البصمة ما تشتغل ولا تظهر، والوقت غلط» — عطلان حقيقيان انحلّوا

فرع `fix/attendance-break-visibility`، **ما انمرج بعد**. لا هجرة، لا تبعية جديدة.
التشخيص كله مقاس على **الإنتاج** (قاعدة البيانات الحية + تشغيل الكود على الجهاز نفسه).

### العطل الأول — تقويم الحضور عند الأدمن يوم كامل للخلف

`attendanceController.dateKey` كان يسوي `new Date(work_date).toISOString().slice(0,10)`.
سيرفر الإنتاج يشتغل بـ**Europe/Berlin**، و`pg` يرجّع عمود `date` كـ JS Date على **منتصف ليل
السيرفر المحلي** — فـ 2026-09-08 يصير `2026-09-08T00:00+02:00` وقراءته بـ UTC تطلع
**2026-09-07**. مبرهن بتشغيله على الصندوق:

```
raw: Tue Sep 08 2026 00:00:00 GMT+0200  →  toISOString().slice(0,10) = 2026-09-07
```

يعني `GET /admin/attendance/calendar` كان يحط أرقام كل يوم **بخانة اليوم اللي قبله**، وخانة
اليوم تقرأ «لا توجد بصمات» وهي فيها ٨ حضور. نفس الفخ اللي مكتوب بعنوان `dateOnly` داخل
`lib/attendanceBreak.js` وانحل هناك من قبل — `attendanceController` ما وصله الحل أبداً.
`salaryController` كان سليم أصلاً لأنه يستعمل `to_char`، ولهذا شاشة الموظف كانت صحيحة
وشاشة الأدمن بس هي الغلط.

**الحل:** مفتاح التجميع صار يجي من Postgres (`to_char(r.work_date, 'YYYY-MM-DD')`)، و`dateKey`
انحذفت. مثبّت بفحص يجبر `process.env.TZ = 'Europe/Berlin'` — لأن العطل **مستحيل يظهر على
صندوق UTC**، و CI يشتغل على UTC.

### العطل الثاني — ضغطة زر خروج مؤقت تنرمي بصمت، وما يعرف بيها ولا واحد

`routes/iclock.js` يرد `OK` على كل رفع — هذا بروتوكول ADMS مو قرارنا — فالجهاز **يصفّر أخضر
على أي إصبع صحيح** والموظف يمشي وهو مصدّق إن ضغطته انسجلت. القرار الحقيقي يصير بعدين داخل
`applyPunch`، وكان يروح لـ`punch_raw.ignored_reason` — عمود **ما يقرأه أي شي**: لا
`listRejects` (هذا يقرأ `punch_reject`، أي السطور اللي ما قدرنا نفهمها)، ولا `listUnmapped`
(أرقام غير مربوطة بس)، ولا تلفون الموظف.

**المقاس على الإنتاج 2026-09-06 → 09-08:** من ١٤ ضغطة زر خروج مؤقت لموظفين مربوطين،
**٥ انرمت بصمت** — كلها «رجعت» (status 5) بلا خروج مفتوح: محمد هيثم ٢ (09-06 23:41،
09-08 20:29) ومضر محمد ٣ (09-07 12:39 و15:58، 09-08 13:52). وردة فعل مضر محفوظة بالبيانات:
بعد ١٩ ثانية من ضغطة 13:52:36 المرمية فتح **طلب خروج من التلفون** — يعني الموظف اكتشف
الخلل بالصدفة.

**الحل:** `lib/attendanceNotices.js` الجديد. `applyPunch` صار يجمع «ملاحظات» بمصفوفة
اختيارية، و`ingestPunches` يرجّعها، و`routes/iclock.js` يوصّلها **بعد الـCOMMIT** (لأن INSERT
فاشل جوّه المعاملة يسقّط دفعة البصمات كلها، ولأن الجهاز يعيد إرسال أي دفعة ما تجيه OK):

| الحدث | الموظف | الأدمن |
|---|---|---|
| ضغط (→) رجعت وما عنده خروج مفتوح | ✅ + شنو يضغط بالضبط | ✅ |
| ضغط (←) أطلع وهو أصلاً بره | ✅ | ✅ |
| خروج مؤقت انفتح من الجهاز | — (هو ضغطه) | ✅ |

الأخير يسد النص الثاني من الشكوى: مسار التلفون يبلّغ الأدمن من زمان (`notifyAdmins`)، ومسار
الجهاز ما كان يبلّغ ولا واحد. مخزون بنوع `attendance_device`، وما بـ`pushOutbox` أي فلترة
حسب النوع فالإشعار يوصل التلفون مثل أي إشعار ثاني.

⚠️ **الأسهم بالنصوص مو زينة.** المالك أكد مرتين (09-06 و09-08): **(←) يبدأ الخروج المؤقت،
(→) ينهيه** — ستيكر المحل، وهو **معكوس عن أسماء ZKTeco الإنكليزية**. كل نص عربي يسمّي الزر
اللي المفروض يضغطه بعدين، فقلبه يعلّم الغلط بدل ما يصلحه.

### اللي **مو** عطل

خريطة `PUNCH_STATE` صحيحة كما هي. مضر ومحمد هيثم يضغطون (→) أول — وهذا ما كان يبين لأن
العطل الثاني كان يخفيه. ما تغيرت الخريطة ولا حرف.

### الفحوصات

`test/attendanceVisibility.test.js` — ٤ فحوص، الأربعة كانوا أحمر قبل التغيير:
تقويم الأدمن · (→) بلا خروج مفتوح يبلّغ الاثنين · (←) مرتين يبلّغ الموظف · خروج من الجهاز
يبلّغ الأدمن. **731/735 بالمجموعة الكاملة**؛ الثلاثة الفاشلة (`noteGroupRouting`،
`pushBroadcast`، وواحد من `adminConsole`) **تفشل على `main` النظيف بنفس الشكل** — انمقاست
قبل وبعد.

### مفتوح — قرارات مالك، مو كود

1. **محمد هيثم مسجّل «بره المحل» من 21:07 اليوم** (خروج `292cc8a4`). المالك راح يسأله.
   لا فلوس تحركت؛ إذا ما انسكر، ينقفل تلقائياً بنهاية دوامه ٢٢:٠٠ ويخسر ~٥٣ دقيقة عمل.
2. **مضر انخصم ١٬٠٠٠ د.ع على خروج مدته ثانيتين** (09-08 19:49:15 → 19:49:17، تلفون،
   «خرجت بدون موافقة» → `approval = 'pending'` → صفر دقيقة مجانية، و`elapsedMinutes` أرضيته
   دقيقة واحدة). قرار المالك: **يترك** — الموافقة على الطلب تصفّر الخصم.
3. **دوام محمد هيثم مخزون ١٥:٠٠–٢٢:٠٠** وهو يبصم دخول ١٠:١٣ / ١٠:١٧ كل يوم مثل الباقين.
   بيانات، مو كود — نفس عائلة دوام مضر ٢٢:١٦ → ١٠:١٥ المفتوح بـ`HANDOFF.md`.

---

## 2026-09-07 (c) — 🔄 تصفير رصيد الخروج المؤقت لأيلول — الجميع رجع على ٥ ساعات كاملة

طلب المالك. **نُفّذ على الإنتاج مباشرة** (`169.58.114.255`)، نسخة احتياطية أولاً:
`/root/loloshop-pre-breakreset-2026-09-07.dump` (7.4 ميغا).

**اللي انعمل:** ١٠ صفوف `state = 'returned'` بشهر `2026-09` صارت `'cancelled'` مع
`admin_note_ar` يشرح السبب. `loadBalance` و`recomputeMonth` يقرأون `'returned'` فقط، فرصيد
كل واحد صار **٠ مستهلك / ٣٠٠ متبقي**. علي اديب ٢ · محمد عادل ٢ · محمد عماد ١ · محمد هيثم ٣ ·
مضر محمد ٣ (برزان ما عنده خروج بأيلول).

**ما تحرك أي دينار:** كل صفوف أيلول كانت `deduction_transaction_id = NULL` و
`deduction_amount = 0` قبل التغيير، ومحسوب بعدها: **صفر** معاملة راتب حية من نوع
`attendance_break` بأيلول. خصم برزان ٣٬٠٠٠ د.ع هو من **آب** (2026-08-30) وانترك مثل ما هو —
المالك قال أيلول فقط.

**مضر محمد لسه بره** (خروج مفتوح من 13:02 اليوم، بسعر ١٬٠٠٠/دقيقة). قبل التصفير كان رصيده
صفر فكل دقيقة محسوبة عليه؛ هسة عنده ٣٠٠ دقيقة مجانية، يعني ما ينخصم شي إذا رجع قبل ~18:02.

**ليش الإلغاء وليس رفع الرصيد:** «رصيد لكل موظف» يسكن على `staff_attendance_user_settings`،
وأول ما يوجد صف بهذا الجدول يصير `is_user_override = true` و**بداية/نهاية الدوام المفردة
بالصف تحل محل جدول الأسبوع بكل الأيام، والجمعة منها** (الدوام ١٠:٠٠–٢٢:٠٠ السبت–الخميس لكن
١٥:٠٠–٠٠:٠٠ الجمعة). أربعة من الستة ما عندهم صف، فإنشاء صف إلهم حتى نخزن الرصيد كان
يعيد كسر عطل الجمعة اللي هجرة 093 موجودة عشانه. التفصيل بلغم جديد بـ`HANDOFF.md`.

**مقاس على الإنتاج قبل التنفيذ:** الرصيد العام ٣٠٠ دقيقة والسعر ١٬٠٠٠/دقيقة، وما في أي
تجاوز غير مضر محمد (٨٥٢) — و**٦٨٤ من الـ٨٥٢ خروج وهمي** (`auto_closed = true`، راح للبيت
الساعة ٢١:٥١ فالنظام فتح خروج بدل ما يقفل يومه) وهو نفس شكل خروج علي اديب ٦٤٠ اللي انلغى
بـ 2026-09-06.

## 2026-09-07 (b) — 🔙 صفحة الطلب كانت تنسى وين كان الموظف · وعاصفة الـSSE

بلاغ برزان: يفتح قطعة من **قائمة الإنتاج**، يحركها من الكوي، فترجعه الصفحة على **لوحته هو**
مو على القائمة اللي كان يشتغل بيها — يخسر المرحلة والفلاتر والبحث ومكانه بالقائمة كل مرة.

**ثلاثة أعطال متراكبة، كل واحد لحاله يكفي يضيّع مكانه.**

**١ — الوجهة غلط.** `app/staff/orders/[orderId]/page.tsx` يحسب `back.href` من `?from=`
(وهذا اللي يستعمله زر «العودة») لكن **كل معالجات الأزرار السبعة كانت تتجاهله** وتكتب
`router.push("/staff")` بالثابت — إكمال · تأكيد تسليم · حذف (كان `/staff/queue`) · إرجاع ·
إرجاع للزبون · موافقة على التصميم · رفض التصميم. برزان يفتح قطعة من **قائمة الإنتاج**،
يحركها من الكوي، فترجعه على **لوحته هو**. صارت كلها تخرج من `back.href`، و`back` انتقل فوق
المعالجات حتى ما ينغلق عليه قبل تعريفه.

**٢ — `router.refresh()` كان يبلع الـpush.** كل معالج كان ينادي `refresh()` وبعده مباشرة
`push()`; الـrefresh يفتح transition والـpush ينضاف عليه فينضاع — **مقاس بالمتصفح: الصفحة
تعلّق على صفحة الطلب مع «تم النقل» ظاهر وما تتحرك أبداً** (انتظرنا ٣٠ ثانية). وأصلاً
الـrefresh هدر: كل قوائم الموظفين تجيب بياناتها بـeffect عن طريق axios، فما في RSC cache
تنفعه. انحذفت من السبعة، والانتقال صار فوري.

**٣ — الاسترجاع كان يدمر المكان اللي يسترجعه.** `app/globals.css:617` يحط
`scroll-behavior: smooth` على `<html>`، فـ`window.scrollTo(0, y)` **ينيمِش**: المتصفح يطلق
`scroll` كل إطار وهو طالع، ومستمع الحفظ بـ`useScrollRestore` يكتب كل موقع وسطي فوق الموقع
الصحيح، وبعدين الانيميشن ينقطع. **مقاس: محفوظ ٧٦٥ → رجعت القائمة على أول الصفحة و
sessionStorage صار ٥٠** — أي نقطة موت الانيميشن، فحتى المحاولة الثانية ما تنفع. الحل:
`behavior: "instant"` + راية `restoring` تسكّت الحفظ لحد ما القفزة تخلص. سيرفر Next نفسه كان
يحذر من هذا بالسجل («Detected `scroll-behavior: smooth` on the <html> element») وقرينا التحذير
على أنه شكلي.

**السرعة — عاصفة أحداث لكل قطعة.** `markEmbroideryZoneBulk` و`advanceBulk` ياخذون **٢٠٠**
عنصر ويلفّون عليهم، وكل دورة تنادي `emitOrderChanged`. كل كونسول موظف كان يعيد تحميل
**كامل** قائمته مرة لكل حدث. القياس اليوم: `GET /api/production/queue` لبرزان =
**٤٬٠٣٤ صف / ٣٫٩٦ ميغا خام (٤٢٧ كيلو مضغوطة)**. يعني ضغطة جملة وحدة على ٤٠ قطعة كانت تسحب
عشرات النسخ من نفس الـ٤ ميغا وتحلّلها بالـJSON على كل تلفون مفتوح. `hooks/useCoalesced.ts`
الجديد يجمع الرشقة بنداء واحد بعد ٤٠٠ms (نفس نمط `/admin` اللي كان مطبّقه لوحده) وانطبق على:
لوحة التجميع · كونسول المحطة · كونسول التجهيز · `/staff` (الطابور والمونيتور). الحضور
(`presence`) ما زال يترقّع محلياً بدون أي جلب.

**نفس عائلة العطل الأول، حالة ثانية:** روابط «تعديل» ما كانت تمرر `?from=`، وصفحة التعديل
ترجع `/staff/orders/:id` بدون `from`. يعني أي تعديل يمسح الأصل: زر «العودة» بعدها يرجع
**لصفحة التعديل نفسها** (لأن البديل هو `document.referrer`)، وأي حركة بعدها تطلع للصفحة
الرئيسية. `editHref()` صار يمرر الأصل، وصفحة التعديل تعيده (وانلفت بـ`Suspense` لأنها صارت
تقرأ `useSearchParams`).

**تحقّق بالمتصفح** (dev، برزان الحقيقي من قاعدة التطوير): `/staff/queue?stage=pressing` →
نزّلنا لـ**٧٦٥px** → فتح قطعة (`?from=%2Fstaff%2Fqueue`) → «إنهاء الكوي، نقل للتجهيز» →
**رجع فوراً على `/staff/queue?stage=pressing`، شريحة «قيد الكوي» نفسها، والعدّاد ٢٠٨٦ →
٢٠٨٥، و`window.scrollY = 765.56` بالضبط.** الأربع كتابات التجريبية كلها انرجعت إلى
`pressing`. `tsc` + `lint` + `next build` نظاف.

⚠️ **درس يستاهل الحفظ:** الفحص الأول بهذي الجلسة قال «الانتقال يشتغل» لأن الضغطة كانت
بـ`javascript_tool` — تبويب Chrome غير المركّز يخنق `rAF` و`setTimeout`، فالنتيجة ما كانت
حقيقية. العطلان ٢ و٣ ما ظهروا إلا لما صارت كل الخطوات بنقرات حقيقية عن طريق `computer`.
