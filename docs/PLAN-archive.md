# PLAN archive — the eleven build phases (2026-05 → 2026-07)

Moved out of `PLAN.md` on 2026-08-05 to cut the per-session context cost. **Nothing was
deleted — this is the original task breakdown, verbatim.**

**All eleven phases shipped.** Verified 2026-08-05 against the tree: every phase has its routes,
controllers and tables on disk (`backend/routes/`, `backend/controllers/`, `db/schema.sql`), and
the site is live at lolo-shop96.com with Android + iOS shells on top of it.

So read this as **history, not a queue** — it records what was asked for and what each task was
told not to do. The `Don't:` lines are the useful part: they are the constraints the shipped code
still honours (cash only, no self-registration, no 3D, server-side price recompute, no cost/profit
exposed to wholesaler or student).

The durable domain model those phases produced — entities, per-product option map, pricing rules —
stayed behind in `PLAN.md`, because it describes the system that exists today.

New work does not get added here. It goes through a spec in `docs/superpowers/specs/`.
...
## APPENDIX — the original v2 model text, verbatim

`PLAN.md` still carries this domain model, but **rewritten**: restated as a description of what
was built, annotated with `db/schema.sql` line numbers, and with the shipped role-price and
package-rule tables named. This appendix is the wording it was rewritten from, kept so the
original phrasing of the 2026-05-25 redefinition is not lost.

Where the two disagree, **`PLAN.md` wins** — it was checked against the schema on 2026-08-05.
