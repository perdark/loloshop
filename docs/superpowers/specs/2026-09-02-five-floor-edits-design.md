...
| D2 | Which rep pieces pass through التجميع? | **Only pieces that carry embroidery** (they are the ones that exist as sub-pieces). Plain robes/caps keep today's birth stage; the شال امريكي keeps its own ladder. | Route plain rep pieces to التجميع too: `fullSetOrder.js` + `cartController.js` birth status change, and the rep-facing «آخر حالة» changes for every plain piece. |
