...
const DPR  = +arg('dpr', '3');
/* iPhone 17 Pro Max is 440x956 pt; the device component draws the 62pt status
   bar itself, so the app viewport captured here is 440x894. */
const W = +arg('w', '440'), H = +arg('h', '894');
...
/* The site is a PWA. Once its service worker installs in this Chrome profile it
   serves the cached offline shell («لا يوجد اتصال بالإنترنت») instead of the live
   page, and every capture silently becomes a screenshot of the offline screen.
   Cost two full failed runs on 2026-08-25. Bypass it always. */
...
for (const p of PAGES) {
  ...
  if (p.clipH) {                       // tall strip for an in-phone scroll
    Object.assign(shot, { captureBeyondViewport: true,
      clip: { x: 0, y: p.clipY ?? 0, width: W, height: p.clipH, scale: 1 } });
  }
