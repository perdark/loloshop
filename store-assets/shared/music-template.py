#!/usr/bin/env python3
"""
TEMPLATE — copy this into your reel folder as music.py and RE-SCORE it.
What to keep: the voices (pad/felt/bell/harp/kick/brush/tick/swell), the reverb,
the master chain. They are tuned and they measure clean.
What to replace: DUR, BEAT, CHORDS, BARS, the three dynamics arcs, SC_A/SC_B, and
every accent in the "accents locked to the picture" block. Add voices if your
direction needs them (strings, marimba, rim) — build them the same way: an
envelope with a >=1.2 ms attack and a real release, then band-limit.
